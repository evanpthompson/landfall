...
void main() {
  ...
  group('applyTheme', () {
    blocTest<ThemeCubit, ThemeState>(
      ...
      build: build,
      setUp: () {
        when(() => repo.applyTheme(2, profileId: any(named: 'profileId')))
            .thenAnswer((_) async {});
        when(() => repo.listThemes()).thenAnswer((_) async => _allThemes);
      },
    ...
    blocTest<ThemeCubit, ThemeState>(
      ...
      build: build,
      setUp: () {
        when(() => repo.applyTheme(any(), profileId: any(named: 'profileId')))
            .thenAnswer((_) async {});
        when(() => repo.listThemes()).thenAnswer((_) async => _allThemes);
      },
      seed: () => ThemeLoaded(_defaultDark, themes: _allThemes),
  ...
  group('ThemeError equality', () {
    test('equal when messages match', () {
      expect(
        const ThemeError('oops'),
        equals(const ThemeError('oops')),
      );
    });
    ...
    test('not equal when messages differ', () {
      expect(
        const ThemeError('a'),
        isNot(equals(const ThemeError('b'))),
      );
    });
  });
}
