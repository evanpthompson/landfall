  ...
  withServerpod(
    ...
    (sessionBuilder, endpoints) {
      ...
      group('authenticated access with bearer token', () {
        test(
          'authenticated session bypasses token check and returns 404 for nonexistent photo',
          () async {
            ...
            expect(
              ...
              equals(404),
              reason:
                  'Authenticated request for nonexistent photo must return 404',
            );
            await session.close();
          },
        );
      ...
      group('signed URL token validation', () {
        test('signed URL lifetime covers a full display restart window', () {
          expect(
            PhotoSigningService.tokenLifetime,
        ...
        test(
          'valid signed URL token and nonexistent photo returns 404',
          () async {
            ...
            final request = RequestInternal.create(
              Method.get,
              Uri.parse(
                'http://localhost/photos/$photoId?token=$token&exp=$exp',
              ),
              Object(),
            );
            final result = await PhotoServeRoute().handleCall(session, request);
            expect(
              ...
              equals(404),
              reason:
                  'Valid signed token with no matching photo must return 404',
            );
            await session.close();
          },
        );
        ...
        test(
          'valid signed URL token for existing photo with no active credential returns 500',
          () async {
