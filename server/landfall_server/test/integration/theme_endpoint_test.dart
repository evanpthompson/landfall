...
void main() {
  withServerpod('Given ThemeEndpoint', (sessionBuilder, endpoints) {
    ...
    group('listThemes', () {
      test('returns at least the built-in themes', () async {
        final themes = await endpoints.theme.listThemes(authed);
    ...
    group('uploadTheme', () {
      test('valid YAML stores and returns the theme', () async {
        final result =
            await endpoints.theme.uploadTheme(authed, _minimalValid);
      ...
      test('uploaded theme appears in listThemes', () async {
        await endpoints.theme.uploadTheme(authed, _fullValid);
        final themes = await endpoints.theme.listThemes(authed);
      ...
      test('invalid YAML returns validation errors without storing', () async {
        final result =
            await endpoints.theme.uploadTheme(authed, _invalidYaml);
    ...
    group('deleteTheme', () {
      test('deletes an imported theme by id', () async {
        ...
        await endpoints.theme.deleteTheme(authed, id);
      ...
      test('throws when attempting to delete a built-in theme', () async {
        ...
        expect(
          () => endpoints.theme.deleteTheme(authed, builtIn.id!),
          throwsA(anything),
        );
      });

      test('is a no-op for unknown id', () async {
        await endpoints.theme.deleteTheme(authed, 999999);
        // Should not throw.
      });
    });
    ...
    group('previewTheme', () {
      test('returns resolved token JSON for a valid theme id', () async {
        ...
        final json = await endpoints.theme.previewTheme(authed, theme.id!);
      ...
      test('throws for unknown theme id', () async {
        expect(
          () => endpoints.theme.previewTheme(authed, 999999),
          throwsA(anything),
        );
      });
    });
    ...
    group('applyTheme', () {
      ...
      test('links a theme to a profile', () async {
        ...
        final profiles = await endpoints.profile.listProfiles(authed);
      ...
      test('applies theme globally when profileId is null', () async {
        ...
        await endpoints.theme.applyTheme(authed, theme.id!);
      });
  ...
  // SEC-02: ThemeEndpoint auth guards (all six methods).
  withServerpod('Given ThemeEndpoint auth guards', (sessionBuilder, endpoints) {
    late TestSessionBuilder authed;
    ...
    test('listThemes rejects unauthenticated caller', () async {
      expect(
        () => endpoints.theme.listThemes(sessionBuilder),
        throwsA(isA<Exception>()),
      );
    });

    test('uploadTheme rejects unauthenticated caller', () async {
      expect(
        () => endpoints.theme.uploadTheme(sessionBuilder, _minimalValid),
        throwsA(isA<Exception>()),
      );
    });

    test('importTheme rejects unauthenticated caller', () async {
      // Auth check must fire before SSRF validation.
      expect(
        () => endpoints.theme.importTheme(
          sessionBuilder,
          'https://example.com/theme.yaml',
        ),
        throwsA(isA<Exception>()),
      );
    });

    test('deleteTheme rejects unauthenticated caller', () async {
      final upload = await endpoints.theme.uploadTheme(authed, _minimalValid);
      expect(
        () => endpoints.theme.deleteTheme(sessionBuilder, upload.theme!.id!),
        throwsA(isA<Exception>()),
      );
    });

    test('previewTheme rejects unauthenticated caller', () async {
      final themes = await endpoints.theme.listThemes(authed);
      expect(
        () => endpoints.theme.previewTheme(sessionBuilder, themes.first.id!),
        throwsA(isA<Exception>()),
      );
    });

    test('applyTheme rejects unauthenticated caller', () async {
      final themes = await endpoints.theme.listThemes(authed);
      expect(
        () => endpoints.theme.applyTheme(sessionBuilder, themes.first.id!),
        throwsA(isA<Exception>()),
      );
    });

    test('authenticated caller retains full read/write access (regression)',
        () async {
      final themes = await endpoints.theme.listThemes(authed);
      expect(themes, isNotEmpty);
      final result = await endpoints.theme.uploadTheme(authed, _minimalValid);
      expect(result.theme, isNotNull);
      final preview =
          await endpoints.theme.previewTheme(authed, result.theme!.id!);
      expect(preview, isNotEmpty);
      await endpoints.theme.deleteTheme(authed, result.theme!.id!);
    });
  });
}
