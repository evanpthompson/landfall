...
- **`landfall-repair.timer`** — every 5 minutes. Targeted recovery for the specific failure modes we've seen: docker daemon stopped → restart it; `.env` missing → log fatal (can't auto-regenerate without invalidating tokens); server container not running → `docker compose up -d`; display crash-looping past 30 exits/day → restart lightdm. Each action is capped per day (`6` docker, `12` compose-up, `4` lightdm) so a genuinely broken state is loud rather than masked.
- **DB integrity probe** (`landfall-db-check.sh`) — runs before each Flutter display launch via the openbox autostart loop. Runs `PRAGMA integrity_check` on the display SQLite DB; if corrupted (e.g. abrupt power loss mid-write), renames it to `landfall.db.corrupted-<ts>` so the app creates a fresh one and resyncs settings from the server. Without this, a single corrupted DB puts the display into a permanent crash loop.
- **KMS GL enforcement** — `dtoverlay=vc4-kms-v3d` and `max_framebuffers=2` are baked into `/boot/firmware/config.txt` at image build time. `landfall-doctor` reports clearly if the active renderer is `llvmpipe` (software) and prints the exact fix.
